<script lang="ts" generics="T extends unknown">
	...
	const checkFilterStatus = (internalName: string, sortingMode: SortingMode) => {
		return $page.url.searchParams.get("orderBy") == internalName && $page.url.searchParams.get("sortingMode") == sortingMode
	}
	...
	{#key $page.url}
		<tr>
			{#each config[gridConfigIndex] as field}
				<th>
					<div class="header">
						{field.fieldName} <div class="actions">
							{#if field.allowSorting}
								{#if checkFilterStatus(field.internalName, SortingMode.ascending)}
									<IconButton on:click={() => setFilter(field.internalName, SortingMode.descending)}>
										<TriangleIcon/>
									</IconButton>
